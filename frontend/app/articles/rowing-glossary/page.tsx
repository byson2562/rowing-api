  ...
  return (
    <main className="site-container lp-page article-page">
      ...
      <article className="article-body">
        ...
        <h2>スイープとスカル</h2>
        <p>
          最初に押さえるのはここだけです。1人でオールを<strong>1本</strong>持つのがスイープ、
        ...
        <p>
          <strong>レート</strong>は1分間に漕ぐ回数(spm)。レース中は32〜40くらい、練習の低レートメニューだと18〜22あたり。「レート20で1時間」と言われたら長い練習の合図です。
        </p>
        <p>
          1本の漕ぎは、オールを水に入れる<strong>キャッチ</strong>、水中で押す<strong>ドライブ</strong>、抜く
          <strong>フィニッシュ</strong>、戻る<strong>リカバリー</strong>の4つに分けて説明されます。「キャッチが遅い」と言われたら、水に入れる瞬間が周りとずれているという意味です。
        </p>
        <p>
        ...
        <p>
          乗る位置にも名前があります。舳先(へさき)側から<strong>バウ</strong>、艫(とも)側の最後尾が
          <strong>ストローク</strong>で、日本語では整調(せいちょう)。整調がリズムを作り、バウ側の漕手がそれに合わせます。
        </p>

        <h2>よくある質問</h2>
        <p>
          <strong>エイトとフォアは何が違う?</strong>
          <br />
          漕手の人数(8人と4人)と、それに伴う速さです。エイトは全種目で最速で、収録データの中では
          {fastest.get("男子エイト")?.time ?? "-"}(男子)。フォアより30秒ほど速く2000mを走ります。
        </p>
        <p>
        ...
        <p>
          <strong>2000mのタイムはどれくらいが速い?</strong>
          <br />
          種目と風で大きく変わるので一概には言えません。目安として、上の表の最速タイムが全日本級の頂点です。自分の艇と比べるなら
          <Link href="/records">歴代記録</Link>や<Link href="/search">検索</Link>
          で、同じ種目・同じ年の順位帯を見るのが早いです。
        </p>
        <p>
        ...
        <p>
          ...
          <Link href="/results">大会結果</Link>から、まず自分の学校を探してみてください。
        </p>
      </article>
    </main>
  );
